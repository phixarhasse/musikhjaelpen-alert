...
import os
import requests
from hueStatehelper import LightState, Color, ColorTemperature, ComplexEncoder, Dimming, Parameters
...
# 65000 - red - turned off - "xy": [0.6758,0.3007]
    ...
    def setBrewingLights(self) -> None:
        ...
        headers = {"hue-application-key": f"{self.username}"}
        try:
            lightState = LightState(Dimming(78.66), Color(0.4931, 0.455), "sparkle", Parameters(Color(0.4931, 0.455), ColorTemperature(153, False), 0.5))
            jsonState = json.dumps(lightState.reprJSON(), cls = ComplexEncoder)
    ...
    def setCoffeeIsDoneLights(self) -> None:
        ...
        headers = {"hue-application-key": f"{self.username}"}
        try:
            brewingState = LightState(Dimming(78.66), Color(0.1673, 0.5968), None, Parameters(Color(0.1673, 0.5968), ColorTemperature(153, False)))
    ...
    def setCoffeeMakerTurnedOff(self) -> None:
        ...
        headers = {"hue-application-key": f"{self.username}"}
        try:
            brewingState = LightState(None, Color(0.6758, 0.3008), None, Parameters(Color(0.6758, 0.3008), ColorTemperature(153, False)))
            jsonState = json.dumps(brewingState.reprJSON(), cls = ComplexEncoder)
